...
class ArticleController extends AdminbaseController{
	...
	function delete(){
		if(isset($_GET['tid'])){
			...
			$data['status']=0;
			$info = $this->term_relationships_model->where("tid=$tid")->delete();
			$article = $this->posts_model->where("id=$tid")->delete();			
			if ( $info && $article ) {
				$this->success("删除成功！");
			} 
			$this->error("删除失败！");
		...
		if(isset($_POST['ids'])){
			$tids=join(",",$_POST['ids']);
			$data['status']=0;
			$info = $this->term_relationships_model->where("tid in ($tids)")->delete();
			$article = $this->posts_model->where("id in ($tids)")->delete();
	...
	function info()
	{
		$id=intval($_GET['id']);
		$posts_model=M("Posts");
		$info = $posts_model->where(['id'=>$id])->find();
		$this->assign($info);
		$this->display();
	}
}